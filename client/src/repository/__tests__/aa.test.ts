...
describe("ProfileRepository", () => {
    ...
    const myUpdateDatasetMock = {"predicates": {"http://www.w3.org/1999/02/22-rdf-syntax-ns#type": {"namedNodes": ["https://github.com/matejikj/diplomka/blob/master/wikimind.ttl#Profile"]}, "http://xmlns.com/foaf/0.1/#term_name": {"literals": {"http://www.w3.org/2001/XMLSchema#string": [""]}}, "http://xmlns.com/foaf/0.1/#term_surname": {"literals": {"http://www.w3.org/2001/XMLSchema#string": [""]}}, "https://github.com/matejikj/diplomka/blob/master/wikimind.ttl#profileImage": {"literals": {"http://www.w3.org/2001/XMLSchema#string": [""]}}, "https://github.com/matejikj/diplomka/blob/master/wikimind.ttl#webId": {"literals": {"http://www.w3.org/2001/XMLSchema#string": ["https://matejikj.datapod.igrant.io/profile/card#me"]}}}, "type": "Subject", "url": "https://inrupt.com/.well-known/sdk-local-node/WikiMind"}
    beforeEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe("updateProfile", () => {
        it("should update profile and return the updated profile", async () => {
            const myDatasetMock = "myDatasetMock";
            const savedProfileSolidDatasetMock = "savedProfileSolidDatasetMock";

            (getSolidDataset as jest.Mock).mockResolvedValue(myDatasetMock);
            (setThing as jest.Mock).mockReturnValue(savedProfileSolidDatasetMock);
            (saveSolidDatasetAt as jest.Mock).mockResolvedValue(undefined);

            const profileRepository = new ProfileRepository();
            const updatedProfile = await profileRepository.updateProfile(profileUrl, profileMock);

            expect(getSolidDataset).toHaveBeenCalledWith(profileUrl, { fetch });
            expect(setThing).toHaveBeenCalledWith(myDatasetMock, myUpdateDatasetMock);
            expect(saveSolidDatasetAt).toHaveBeenCalledWith(profileUrl, savedProfileSolidDatasetMock, { fetch });
            expect(updatedProfile).toEqual(profileMock);
        });
    });
});
