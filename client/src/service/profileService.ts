...
export async function createProfile(sessionId: string, profile: Profile) {
    const profiles = await getProfileAll(sessionId, { fetch });

    let myExtendedProfile = profiles.altProfileAll[0];
    let userDataThing = getThing(myExtendedProfile, sessionId);
...
export async function getProfile(sessionId: string) {
    ...
    if (bb === null) {
        createProfile(sessionId, {name: "jakub", surname: "matejik"})
    } else {
