...
describe("MindMapService", () => {
  ...
  describe("createNewMindMap", () => {
    it("should create a new mind map and return its URL", async () => {
      ...
      // // Check if the mind map data is saved correctly in the datasets
      // const mindMapsDataset = await getSolidDataset(mindMapsDatasetUrl, { fetch });
      // const mindMapThing = await getThing(mindMapsDataset, createdMindMapUrl);
      // const mindMapData = mindMapLDO.read(mindMapThing);

      // expect(mindMapData).toBeDefined();
      // expect(mindMapData?.name).toEqual(mindMapName);
      // expect(mindMapData?.storage).toEqual(mindMapStorageUrl);
      // expect(mindMapData?.source).toEqual(userSession.podUrl);

      // // Check if the mind map link is added to the mindMaps dataset
      // const linksDataset = await getSolidDataset(mindMapsDatasetUrl, { fetch });
      // const linkThing = await getThing(linksDataset, createdMindMapUrl);
      // const linkData = linkLDO.read(linkThing);

      // expect(linkData).toBeDefined();
      // expect(linkData?.linkType).toEqual(LinkType.GRAPH_LINK);
    });
  });
